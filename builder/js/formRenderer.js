...
function formRenderer(objOS_a, strFormID_a, objParameters_a)
{
    ...
    function renderField(objField_a, strContainerName_a)
    {
        ...
        if (objField_a.visible === undefined || toBoolean(objField_a.visible))
        {
            switch (strType)
            {
                case 'textbox':
    ...
    function renderContainer(objContainer_a, blnShowCaption_a)
    {
        var strHTML = '';
        ...
        if (objContainer_a.visible === undefined || toBoolean(objContainer_a.visible))
        {
            if (strType === 'horizontalcontainer')
            {
                strHTML += '<div class="gscr-container gscr-containerHorizontal">';

                if (blnShowCaption_a && strCaption.length > 0)
                {
                ...
                for (var intI = 0; intI < arrChildren.length; intI++)
                {
                    ...
                    {
                        strHTML += renderContainer(objChild, true);
                    }
                    else
                    {
                        strHTML += renderField(objChild, strContainerName, blnShowCaption_a);
                    }
            ...
            {
                strHTML += '<div class="gscr-container gscr-containerVertical">';
                ...
                for (var intJ = 0; intJ < arrChildren.length; intJ++)
                {
                    ...
                    {
                        strHTML += renderContainer(objChild2, true);
                    }
                    else
                    {
    ...
    function renderToolbar(objSection_a)
    {
        ...
        for (var intI = 0; intI < arrContainers.length; intI++)
        {
            ...
            for (var intJ = 0; intJ < arrChildren.length; intJ++)
            {
                ...
                {
                    strHTML += renderField_ToolbarButton(objBtn, true);
                }
            }
        }
	...
	function renderSection(objSection_a, blnShowCaption_a)
	{
		var strHTML       = '';
		...
		for (var intI = 0; intI < arrContainers.length; intI++)
		{
			strHTML += renderContainer(arrContainers[intI], blnShowCaption_a);
		}
    ...
    function renderForm()
    {
        ...
        for (intI = 0; intI < m_arrLayoutSections.length; intI++)
        {
			...
			if (toBoolean(objSection.visible))
			{
				...
				{
					strBodyHTML += renderSection(objSection, intVisibleCount > 1);
				}
			}
        }
		...
		for (var intI = 0; intI < arrChildren.length; intI++)
		{
			...
			else
			{
				...
				if (objElement.length > 0)
				{
					...
					{
						objChild.value = objElement.is(':checked') ? true : false;
					}
					else
					{
				...
				else if (objChild.type === 'checkbox')
				{
					objChild.value = toBoolean(objChild.value) ? true : false;
				}
			}
		}
	}
	...
	this.validate = function()
	{
		...
		function getFieldValue(objField_a, objElement_a)
		{
			...
			{
				varResult = objElement_a.is(':checked');
			}
			else
			{
				varResult = $.trim(objElement_a.val());
			}
		...
		for (var intS = 0; intS < m_arrLayoutSections.length; intS++)
		{
			...
			if (objSection.containers)
			{
				for (var intC = 0; intC < objSection.containers.length; intC++)
				{
					...
					if (objContainer.children)
					{
						traverseChildren(objContainer.children, objContainer.name);
					}
				}
			}
		}

		function traverseChildren(arrChildren_a, strContainerName_a)
		{
			for (var intF = 0; intF < arrChildren_a.length; intF++)
			{
				var objField = arrChildren_a[intF];
